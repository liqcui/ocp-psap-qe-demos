...
if [ $? -eq 0 ];then
     ...
     #display_and_run "aws s3api create-bucket --bucket $BUCKET_NAME"
     display_and_run "aws s3 mb s3://psap-qe-ocps3bucket --region us-east-2"
     display_and_run "aws s3api delete-public-access-block --bucket $BUCKET_NAME"
echo '{
  "Version": "2012-10-17",
  "Statement": [
    {
     ...
     cat policy.json
     display_and_run "aws s3api put-bucket-policy --bucket $BUCKET_NAME --policy file://policy.json"
