...
if [ $? -eq 0 ];then
     print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
     print_stdout_withcolor yellow `repeatedCharNTimes "The Nodepool <hugepages-nodepool> Has Been Created" 86`
     print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
else
     display_and_run "oc create -f hugepages-nodepool.yaml"
fi


print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
show_prompt_text yellow "It will take 5-10 minutes to complete, waiting for new custom nodepool be ready"
print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
...
print_stdout_withcolor green `repeatedCharNTimes "-" 86`
display_and_run "for node in \`oc get nodes -oname\`;do oc debug \$node --quiet=true -n  openshift-cluster-node-tuning-operator -- chroot /host cat /proc/cmdline | grep hugepagesz; done"
echo
...
print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
show_prompt_text yellow "It will take 5-10 minutes to complete, waiting for the status of custom nodepool be ready"
print_stdout_withcolor yellow `repeatedCharNTimes "-" 86`
